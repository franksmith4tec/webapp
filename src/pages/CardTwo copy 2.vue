...
<div v-if="!searchMode" class="tab-filters">
  <div class="tab-container">
    ...
    <button 
      class="tab-button" 
      :class="{ 'active': activeTab === 'pending-failed' }"
...
const filteredData = computed(() => {
  if (searchMode.value) return displayData.value
...
const performSearch = () => {
  ...
  if (allData.value.length > 0) {
    ...
    // Search by payment_id OR customer_name OR customer_email
    let results = allData.value.filter(item => {
      const name = (item.customer_name || '').toLowerCase()
      const paymentId = (item.payment_id || '').toLowerCase()
      const email = (item.customer_email || '').toLowerCase() // ADD THIS LINE
      return name.includes(query) || paymentId.includes(query) || email.includes(query) // MODIFIED THIS LINE
    })
    ...
    results = results.sort((a, b) => {
      ...
      const paymentIdA = (a.payment_id || '').toLowerCase()
      const paymentIdB = (b.payment_id || '').toLowerCase()
      const emailA = (a.customer_email || '').toLowerCase() // ADD THIS LINE
      const emailB = (b.customer_email || '').toLowerCase() // ADD THIS LINE
      const query = searchQuery.value.toLowerCase().trim()
      ...
      // Exact match on email (ADD THIS BLOCK)
